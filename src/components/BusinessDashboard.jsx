...
import { cn, formatCurrency, formatOrderItems, getBusinessProductRates, getOrderDetailsPath, getPaymentStatusMeta, hasCustomProductRates } from '../lib/utils';
    ...
    <div className="space-y-6">
      <div className="bg-orange-600 p-6 rounded-3xl text-white shadow-lg relative overflow-hidden">
        <div className="relative z-10">
          <p className="text-orange-100 text-sm font-medium">{t('business.welcome', 'Welcome back,')}</p>
          <h2 className="text-2xl font-bold">{profile?.businessName}</h2>
          <div className="mt-4 flex flex-col gap-4">
            <div>
              <p className="text-orange-100 text-xs uppercase tracking-wider">{t('business.trustedSupply', 'Trusted Supply')}</p>
              <p className="text-lg font-bold">{t('business.heroTitle', 'Fresh. Reliable. Always On Time.')}</p>
              <p className="text-orange-100 text-xs mt-1">{t('business.heroSubtitle', 'Delivering premium quality chicken you can trust every single day.')}</p>
            </div>
            <Link
              to="/business/order"
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
          ...
          <p className="text-gray-500 text-xs font-medium">{t('business.pendingOrders', 'Pending Orders')}</p>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
          ...
          <p className="text-gray-500 text-xs font-medium">{t('business.outstanding', 'Outstanding')}</p>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <h3 className="font-bold text-gray-900">{t('business.recentOrders', 'Recent Orders')}</h3>
          <div className="flex items-center gap-2">
            ...
            <Link to="/business/history" className="text-orange-600 text-sm font-medium flex items-center">
              {t('business.viewAll', 'View all')} <ChevronRight size={16} />
            </Link>
          </div>
        </div>
          ...
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl text-sm focus:ring-orange-500 focus:border-orange-500" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-500 uppercase flex items-center gap-1">
                  <CalendarRange size={12} /> {t('common.dateFrom', 'Date From')}
                </label>
                <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl text-sm focus:ring-orange-500 focus:border-orange-500" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-500 uppercase flex items-center gap-1">
                  <CalendarRange size={12} /> {t('common.dateTo', 'Date To')}
                </label>
                <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl text-sm focus:ring-orange-500 focus:border-orange-500" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-500 uppercase flex items-center gap-1">
                  <IndianRupee size={12} /> {t('common.minPrice', 'Min Price')}
                </label>
                <input type="number" min="0" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl text-sm focus:ring-orange-500 focus:border-orange-500" />
              </div>
              <div className="space-y-1 sm:col-span-2">
                <label className="text-xs font-bold text-gray-500 uppercase flex items-center gap-1">
                  <IndianRupee size={12} /> {t('common.maxPrice', 'Max Price')}
                </label>
                <input type="number" min="0" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl text-sm focus:ring-orange-500 focus:border-orange-500" />
              </div>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-center py-10 bg-white rounded-2xl border border-dashed border-gray-300">
            ...
            <p className="text-gray-500 text-sm">{t('business.noOrders', 'No orders match these filters')}</p>
          </div>
        ) : (
          <div className="space-y-3">
      ...
      >
        ...
        {t('business.support', 'Support on WhatsApp')}
      </button>
    </div>
  );
}
