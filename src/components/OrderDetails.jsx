        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {(order.items || []).map((item) => (
              <div key={item.type} className="bg-gray-50 rounded-2xl p-3 flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-bold text-gray-900">{getProductLabel(item.type)}</p>
                  <p className="text-xs text-gray-500">{formatCurrency(item.pricePerKg || 0)}/kg</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-gray-900">{item.quantity || 0} kg</p>
                  <p className="text-xs text-gray-500">{formatCurrency((item.quantity || 0) * (item.pricePerKg || 0))}</p>
                </div>
              </div>
            ))}
          </div>
