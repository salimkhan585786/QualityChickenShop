...
const WEEKDAYS = [
  { id: 'monday', labelKey: 'weekday.mon', fallback: 'Mon' },
  { id: 'tuesday', labelKey: 'weekday.tue', fallback: 'Tue' },
  { id: 'wednesday', labelKey: 'weekday.wed', fallback: 'Wed' },
  { id: 'thursday', labelKey: 'weekday.thu', fallback: 'Thu' },
  { id: 'friday', labelKey: 'weekday.fri', fallback: 'Fri' },
  { id: 'saturday', labelKey: 'weekday.sat', fallback: 'Sat' },
  { id: 'sunday', labelKey: 'weekday.sun', fallback: 'Sun' },
];
...
export default function Subscriptions() {
  ...
  useEffect(() => {
    ...
    const subscriptionsQuery = query(collection(db, 'subscriptions'), where('customerId', '==', user.uid));
  ...
  const productRates = useMemo(() => getBusinessProductRates(settings, profile), [settings, profile]);
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={submitSubscription} className="space-y-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
          ...
          {PRODUCT_DEFINITIONS.map((product) => {
            ...
            return (
              <button
                ...
                className={`rounded-2xl border p-3 text-sm text-center transition-colors ${selected ? 'border-orange-500 bg-orange-50 text-orange-700' : 'border-gray-200 bg-white'}`}
              >
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {WEEKDAYS.map((day) => (
              <button
                ...
                className={`rounded-xl px-3 py-2 text-sm font-bold border ${form.selectedDays.includes(day.id) ? 'border-orange-500 bg-orange-50 text-orange-700' : 'border-gray-200 bg-white text-gray-600'}`}
              >
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-bold uppercase text-gray-500">{t('subscription.timeSlot', 'Time Slot')}</label>
            <select value={form.timeSlot} onChange={(e) => setForm((current) => ({ ...current, timeSlot: e.target.value }))} className="mt-1 w-full rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm">
          ...
          <div>
            <label className="text-xs font-bold uppercase text-gray-500">{t('subscription.notes', 'Notes')}</label>
            <input
              value={form.notes}
              onChange={(e) => setForm((current) => ({ ...current, notes: e.target.value }))}
              className="mt-1 w-full rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm"
              placeholder={t('subscription.notesPlaceholder', 'Optional notes')}
            />
          </div>
        </div>

        <div className="flex gap-2">
          <button type="submit" disabled={loading} className="flex-1 rounded-2xl bg-orange-600 px-4 py-3 font-bold text-white disabled:opacity-50">
            {loading ? t('common.save', 'Save') : form.id ? t('subscription.saveEdit', 'Update Subscription') : t('subscription.saveNew', 'Create Subscription')}
          </button>
          {form.id && (
            <button type="button" onClick={() => setForm(emptyForm)} className="rounded-2xl border border-gray-200 px-4 py-3 font-bold text-gray-600">
        ...
        {subscriptions.length === 0 ? (
          ...
          <div key={subscription.id} className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-bold text-gray-900">{subscription.active ? t('subscription.active', 'Active') : t('subscription.cancelled', 'Cancelled')}</p>
              ...
              <div className="flex gap-2">
                {subscription.active && (
                  <button type="button" onClick={() => editSubscription(subscription)} className="rounded-xl border border-orange-200 bg-orange-50 px-3 py-2 text-xs font-bold text-orange-700">
                    {t('subscription.edit', 'Edit Subscription')}
                  </button>
                )}
                {subscription.active && (
                  <button type="button" onClick={() => cancelSubscription(subscription.id)} className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-bold text-red-700">
